...
//#undef _FNC
//#define _FNC "Hello world"

#ifdef _FNC
//#define FNC "in " STR(_FNC)
#define FNC _FNC
#else
#define FNC NULL
#endif

static void notify_failure(
...
static inline int start_tx_task(struct cal_tx_task *task)
{
    ...
    TRUEDO(status != 0, return BLADERF_ERR_UNEXPECTED) else {
        task->started = true;
        return 0;
    }
}
...
int calibrate_dc(struct cli_state *s, unsigned int ops)
{
    ...
    if (IS_CAL(CAL_DC_LMS_TUNING, ops)) {
        ...
        TRUEDO(status != 0, goto error) else {
